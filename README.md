...
4. Download the app from the [release](https://github.com/hotwax/order-routing/releases) page, extract it, and place it at `apps/order-routing`.
5. Create a `.env` file in `apps/order-routing` by taking reference from its `.env.example`.
...
4. Clone this app under `apps/` using the command:  
    `git clone https://github.com/hotwax/order-routing.git apps/order-routing`
5. Create a `.env` file in `apps/order-routing` by taking reference from its `.env.example`.
6. Run following command from the `accxui` root to download dependencies  
    `pnpm install`
7. To run the app in browser use the command from the `accxui` root:  
...
2. Pick an issue from [here](https://github.com/hotwax/order-routing-rules/issues). Write in the issue comment that you want to pick it, if you can't assign yourself. **Please stay assigned to one issue at a time to not block others**.
